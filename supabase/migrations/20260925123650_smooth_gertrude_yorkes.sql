...
-- Coach review inbox: add p_search (student name, essay title, book / source
-- title; case-insensitive substring) and compute counts + the page in a single
-- pass instead of running the whole CTE pipeline twice. rev_max is now scoped
-- to the in-scope essays instead of scanning every revision.
-- The old 8-arg signature is dropped: a new trailing default parameter would
-- otherwise make 8-arg calls ambiguous between the two overloads.
drop function if exists public.coach_review_filtered_ids(uuid, uuid, text, text, text, text, int, int);
...
comment on function public.coach_review_filtered_ids(uuid, uuid, text, text, text, text, int, int, text) is 'Coach review inbox: returns paginated essay ids plus exact unread/read totals for all filters (team, rocket, points, reply, search, tab). Read status is team-wide.';
