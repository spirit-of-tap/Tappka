...
-- Read status is team-wide: an essay counts as read once ANY coach/admin has
-- marked it, so it moves to the "Přečtené" tab for every coach. Previously
-- read_ids only held the caller's own reads (per-coach inbox).
