-- Migration: Fix validate_picture_only_update trigger to allow system operations
-- Problem: The trigger was blocking ALL updates to profiles table, including:
--   1. FK cascade when user is deleted (SET NULL on user_id)
--   2. Profile linking when user verifies their school email
-- Solution: Allow system operations and user_id changes, only restrict user updates to picture column
