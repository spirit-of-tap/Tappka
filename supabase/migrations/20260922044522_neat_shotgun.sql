...
-- Allow profile owners to update bio_json: bio_json changes no longer trigger
-- rejection in validate_picture_only_update(); error message updated accordingly.
-- The user_id-linking branch requires bio_json to stay unchanged (like all other
-- non-linking columns) so linking workflows can't smuggle bio edits.
...
begin
  ...
  if old.user_id is distinct from new.user_id then
    if (
      ...
      and old.former_team_id is not distinct from new.former_team_id
      and old.team_left_at is not distinct from new.team_left_at
      and old.team_removed_by_profile_id is not distinct from new.team_removed_by_profile_id
  ...
  -- regular users may only change picture, bio_json and beta_access_granted_at
  if (
    ...
    or old.former_team_id is distinct from new.former_team_id
    or old.team_left_at is distinct from new.team_left_at
    or old.team_removed_by_profile_id is distinct from new.team_removed_by_profile_id
    ...
    raise exception 'Only picture, bio_json and beta_access_granted_at can be updated by users';
  end if;
