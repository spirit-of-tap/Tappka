...
DROP FUNCTION public.birth_giving_confirm_assignment(uuid, text, text, text, bigint);
DROP FUNCTION public.birth_giving_confirm_result_file(uuid, uuid, text, text, text, bigint);

CREATE FUNCTION public.birth_giving_confirm_assignment(
...
CREATE FUNCTION public.birth_giving_confirm_result_file(
...
CREATE FUNCTION public.birth_giving_unreferenced_storage_paths(p_grace_period interval)
RETURNS SETOF text
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO ''
AS $$
BEGIN
  IF p_grace_period IS NULL OR p_grace_period <= interval '0 seconds' THEN
    RAISE EXCEPTION 'A positive cleanup grace period is required' USING ERRCODE = '22023';
  END IF;
  RETURN QUERY
  SELECT object.name FROM storage.objects object
  WHERE object.bucket_id = 'documents'
    AND object.name LIKE 'birth-giving/%'
...
REVOKE ALL ON FUNCTION public.birth_giving_confirm_assignment(uuid, uuid, text, text, text, bigint) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.birth_giving_confirm_result_file(uuid, uuid, uuid, text, text, text, bigint) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.birth_giving_unreferenced_storage_paths(interval) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.birth_giving_confirm_assignment(uuid, uuid, text, text, text, bigint) TO service_role;
GRANT EXECUTE ON FUNCTION public.birth_giving_confirm_result_file(uuid, uuid, uuid, text, text, text, bigint) TO service_role;
GRANT EXECUTE ON FUNCTION public.birth_giving_unreferenced_storage_paths(interval) TO service_role;
