CREATE FUNCTION public.birth_giving_find_event_conflict(
...
AS $function$
BEGIN
  IF public.birth_giving_active_profile_id() IS NULL THEN
...
REVOKE ALL ON FUNCTION public.birth_giving_find_event_conflict(text, text, timestamp with time zone) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.birth_giving_find_event_conflict(text, text, timestamp with time zone) TO authenticated;
...
BEGIN
  ...
  IF v_event.status NOT IN ('draft', 'published')
     OR (v_event.status = 'published' AND v_now >= v_event.starts_at + (CASE v_event.duration
  ...
  IF v_event.status = 'published'
     AND v_event.starts_at > v_now
     AND p_starts_at IS NOT NULL
     AND p_starts_at <= v_now THEN
    RAISE EXCEPTION 'Published event cannot be rescheduled to its start or the past' USING ERRCODE = '55000';
  END IF;
