...
export const birthGivingTeams = pgTable("birth_giving_teams", {
  ...
  pgPolicy("BG organizers can insert teams", { for: "insert", to: ["authenticated"], withCheck: sql`EXISTS (SELECT 1 FROM birth_giving_event_organizers o WHERE o.event_id = event_id AND o.profile_id = current_profile_id()) AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id()` }),
  pgPolicy("BG organizers can update teams", { for: "update", to: ["authenticated"], using: sql`EXISTS (SELECT 1 FROM birth_giving_event_organizers o WHERE o.event_id = event_id AND o.profile_id = current_profile_id())`, withCheck: sql`EXISTS (SELECT 1 FROM birth_giving_event_organizers o WHERE o.event_id = event_id AND o.profile_id = current_profile_id()) AND updated_by_profile_id = current_profile_id()` }),
