...
export const birthGivingReflections = pgTable("birth_giving_reflections", {
  ...
  pgPolicy("Participants can create their BG reflections", { for: "insert", to: ["authenticated"], withCheck: sql`${activeCaller} AND profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id()` }),
  pgPolicy("Participants can update their BG reflections", { for: "update", to: ["authenticated"], using: sql`${activeCaller} AND profile_id = current_profile_id()`, withCheck: sql`${activeCaller} AND profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id()` }),
