...
export const birthGivingEvents = pgTable("birth_giving_events", {
  ...
  check("birth_giving_events_normalized_name_check", sql`normalized_name = lower(regexp_replace(trim(name), '[[:space:]]+', ' ', 'g'))`),
...
export const birthGivingTeams = pgTable("birth_giving_teams", {
  ...
  pgPolicy("Community can view published BG teams", { for: "select", to: ["authenticated"], using: sql`EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.removed_at IS NULL AND ((e.status = 'published' AND ${verifiedCommunity}) OR EXISTS (SELECT 1 FROM birth_giving_event_organizers o WHERE o.event_id = e.id AND o.profile_id = current_profile_id())))` }),
...
export const birthGivingLookingForTeam = pgTable("birth_giving_looking_for_team", {
  ...
  pgPolicy("Community can view BG team searches", { for: "select", to: ["authenticated"], using: sql`${verifiedCommunity} AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.removed_at IS NULL)` }),
  pgPolicy("Profiles can start their own BG team search", { for: "insert", to: ["authenticated"], withCheck: sql`profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id() AND EXISTS (SELECT 1 FROM profiles caller_profile WHERE caller_profile.id = current_profile_id() AND caller_profile.access_removed_at IS NULL) AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.joining_open AND now() < e.starts_at AND e.removed_at IS NULL) AND NOT EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.profile_id = current_profile_id())` }),
  pgPolicy("Profiles can update their own BG team search", { for: "update", to: ["authenticated"], using: sql`profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id() AND EXISTS (SELECT 1 FROM profiles caller_profile WHERE caller_profile.id = current_profile_id() AND caller_profile.access_removed_at IS NULL) AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.joining_open AND now() < e.starts_at AND e.removed_at IS NULL) AND NOT EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.profile_id = current_profile_id())`, withCheck: sql`profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id() AND EXISTS (SELECT 1 FROM profiles caller_profile WHERE caller_profile.id = current_profile_id() AND caller_profile.access_removed_at IS NULL) AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.joining_open AND now() < e.starts_at AND e.removed_at IS NULL) AND NOT EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.profile_id = current_profile_id())` }),
  pgPolicy("Profiles can stop their own BG team search", { for: "delete", to: ["authenticated"], using: sql`profile_id = current_profile_id()` }),
]).enableRLS()
...
export const birthGivingTeamResultFiles = pgTable("birth_giving_team_result_files", {
  ...
  pgPolicy("Community can view published BG result files", { for: "select", to: ["authenticated"], using: sql`removed_at IS NULL AND ${verifiedCommunity} AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.removed_at IS NULL)` }),
  pgPolicy("BG members and organizers can insert result files", { for: "insert", to: ["authenticated"], withCheck: sql`uploaded_by_profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id() AND (EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.team_id = team_id AND m.profile_id = current_profile_id()) OR EXISTS (SELECT 1 FROM birth_giving_event_organizers o JOIN birth_giving_events e ON e.id = o.event_id WHERE o.event_id = event_id AND o.profile_id = current_profile_id() AND e.starts_at + CASE e.duration WHEN '8h' THEN interval '8 hours' ELSE interval '24 hours' END <= now()))` }),
  pgPolicy("BG members and organizers can update result files", { for: "update", to: ["authenticated"], using: sql`EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.team_id = team_id AND m.profile_id = current_profile_id()) OR EXISTS (SELECT 1 FROM birth_giving_event_organizers o JOIN birth_giving_events e ON e.id = o.event_id WHERE o.event_id = event_id AND o.profile_id = current_profile_id() AND e.starts_at + CASE e.duration WHEN '8h' THEN interval '8 hours' ELSE interval '24 hours' END <= now())`, withCheck: sql`updated_by_profile_id = current_profile_id() AND (EXISTS (SELECT 1 FROM birth_giving_team_members m WHERE m.event_id = event_id AND m.team_id = team_id AND m.profile_id = current_profile_id()) OR EXISTS (SELECT 1 FROM birth_giving_event_organizers o JOIN birth_giving_events e ON e.id = o.event_id WHERE o.event_id = event_id AND o.profile_id = current_profile_id() AND e.starts_at + CASE e.duration WHEN '8h' THEN interval '8 hours' ELSE interval '24 hours' END <= now()))` }),
...
export const birthGivingReflections = pgTable("birth_giving_reflections", {
  ...
  pgPolicy("Community can view published BG reflections", { for: "select", to: ["authenticated"], using: sql`removed_at IS NULL AND ${verifiedCommunity} AND EXISTS (SELECT 1 FROM birth_giving_events e WHERE e.id = event_id AND e.status = 'published' AND e.removed_at IS NULL)` }),
  pgPolicy("Participants can create their BG reflections", { for: "insert", to: ["authenticated"], withCheck: sql`profile_id = current_profile_id() AND created_by_profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id()` }),
  pgPolicy("Participants can update their BG reflections", { for: "update", to: ["authenticated"], using: sql`profile_id = current_profile_id()`, withCheck: sql`profile_id = current_profile_id() AND updated_by_profile_id = current_profile_id()` }),
