...
export async function POST(_request: Request, { params }: RouteContext) {
  ...
  const { data: paths, error } = await context.supabase.rpc("birth_giving_mark_result_missing", {
    p_event_id: eventId,
    p_team_id: teamId,
  });
  if (error) return birthGivingMutationErrorResponse(error, context.supabase, eventId);
  await Promise.all((paths ?? []).map((path) => deleteFile("documents", path).catch((deleteError: unknown) => {
    console.error("Failed to delete missing BG result object:", deleteError);
  })));
